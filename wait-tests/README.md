...
| h | `set_parallel(); child(fast) as B; child(slow) as C; wait_all(); ok` | `E.005.020.started_at` (WAIT-B) earlier than `E.010.005.completed_at` (C's slow); `E.005.025.completed_at` earlier than `E.010.005.completed_at`; `E.015.started_at` (WAIT-A) later than `E.010.015.completed_at`; job `completed`. WAIT-B later than C's slow = scope leaked = FAIL |
