...
| e | `set_parallel(); wait_test_child(slow); wait_all(); ok` | `E.005` is A; `E.010.005.005` is A-child; `E.015.started_at` (WAIT) later than `E.010.005.010.completed_at` (the grandchild slow order) |
| g | `slow; set_parallel(); ok; ok; wait_all(); ok` | `E.010` is A; `E.025.started_at` later than `E.005.completed_at` |
| h | `set_parallel(); child(fast) as B; child(slow) as C; wait_all(); ok` | `E.005` is A, `E.010.005.005` is A-B, and `E.015.005.005` is A-C; `E.010.005.025.started_at` (WAIT-B) earlier than `E.015.005.010.completed_at` (C's slow); `E.020.started_at` (WAIT-A) not earlier than every `E.*.completed_at` (compare full-precision timestamps); job `completed`. B's post-wait order finishing before C's slow is a race, not a pass condition. WAIT-B later than C's slow = scope leaked = FAIL |
